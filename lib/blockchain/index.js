...
class Blockchain {
    ...
    init() {
        ...
        if (this.teachers.length == 0){
            const teacher = new Teacher('12345678t'); 
    ...
    getAttendanceTransactions({ User_ID, eventID, startTime, endTime }) {
        console.log('getAttendanceTransactions called');
        console.log('Original parameters:', { User_ID, eventID, startTime, endTime });

        const transactionsInBlocks = R.flatten(R.map(R.prop('transactions'), this.getAllBlocks()));    
        const parsedStartTime = this.convertToTimestamp(startTime);
        const parsedEndTime = this.convertToTimestamp(endTime);
        console.log('Parsed timestamps:', { parsedStartTime, parsedEndTime });
        ...
        return transactionsInBlocks.filter(tx => {
            ...
            console.log('Transaction:', tx.id, 'isStudentMatch:', isStudentMatch, 'isEventMatch:', isEventMatch, 'isTimeMatch:', isTimeMatch);
