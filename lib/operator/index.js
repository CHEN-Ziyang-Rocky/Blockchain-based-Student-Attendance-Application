...
class Operator {
    ...
    async Student_register(studentID, password) {
        const newWallet = this.createWalletFromPassword(password);
        const publicKeyAddress = this.generateAddressForWallet(newWallet.id);

        //console.log(`Student ${studentID} registered as a miner with wallet ID: ${newWallet.id} and address: ${publicKeyAddress}`);
        ...
        console.log(`Student ${studentID} registered with wallet ID: ${newWallet.id} and public address: ${publicKeyAddress}`);
    ...
    async recordAttendance(studentId, eventId, timestamp, signature) {
        const attendanceTransaction = {
            id: `${studentId}_${eventId}_${timestamp}`,
            type: 'attendance',
            data: {
                inputs: [],                   
                outputs: [                    
        ...
        try {
            ...
            console.log(`Attendance recorded for student ID: ${studentId}, event ID: ${eventId} at timestamp: ${timestamp}`);
