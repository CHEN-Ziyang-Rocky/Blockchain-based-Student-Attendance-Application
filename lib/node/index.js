...
class Node {
    ...
    async checkReceivedBlock(block) {
        return await this.checkReceivedBlocks([block]);
    }

    async checkReceivedBlocks(blocks) {
        ...
        if (latestBlockHeld.hash === latestBlockReceived.previousHash) { // We can append the received block to our chain
            ...
            await this.blockchain.replaceChain(receivedBlocks);
            return true;
        }
    }
}
