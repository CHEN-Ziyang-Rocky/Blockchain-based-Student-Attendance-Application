...
class Miner {
    ...
    mine(rewardAddress, feeAddress) {
        ...
        const promise = thread.promise().then((result) => {
            console.log("Mined block transactions:", JSON.stringify(result.transactions, null, 2));
            thread.kill();
            //this.blockchain.addBlock(result);

            // this.blockchain.transactions = this.blockchain.transactions.filter(tx => 
            // !result.transactions.some(blockTx => blockTx.id === tx.id)
            // );
            // this.blockchain.transactionsDb.write(this.blockchain.transactions);

            // console.info(`Block successfully mined and added. Transactions list updated.`);
    ...
    static verifyEventTransaction(transaction, blockchain) {
        const currentTime = new Date().getTime();
    
        if (currentTime > transaction.data.outputs[0].timestamp) {
        ...
        console.log(transaction.data.outputs[0].publicKey);
