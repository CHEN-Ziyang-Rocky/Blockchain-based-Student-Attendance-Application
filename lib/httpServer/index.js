...
class HttpServer {
    constructor(node, blockchain, operator, miner) {
        ...
        // this.app.get('/blockchain/transactions/attendance', (req, res) => {        
        //     try {
        ...
        //         const attendanceTransactions = blocks
        //             .flatMap(block => block.transactions)
        //             .filter(transaction => transaction.type === 'attendance');
                
        //         const filteredTransactions = attendanceTransactions.filter(transaction => {
        //             const output = transaction.data.outputs[0];
        //             let matches = true;

        //             if (User_ID) {
        //                 matches = matches && output.User_ID === User_ID;
        //             }
        //             if (eventID) {
        //                 matches = matches && output.eventID === eventID;
        //             }
        //             if (startTime) {
        //                 matches = matches && output.timestamp >= parseInt(startTime, 10);
        //             }
        //             if (endTime) {
        //                 matches = matches && output.timestamp <= parseInt(endTime, 10);
        //             }
        ...
        //             return res.status(404).json({ message: "No attendance transactions found for the specified criteria." });
        //         }
        
        //         res.status(200).json(filteredTransactions);
        //     } catch (error) {
        //         console.error("Error fetching attendance transactions:", error.message);
        //         res.status(500).json({ message: "Failed to fetch attendance transactions", error: error.message });
        //     }
        // });  
        this.app.get('/blockchain/transactions/attendance', (req, res) => {    
            console.log('API called: /blockchain/transactions/attendance');
            console.log('Query parameters:', req.query);    
            try {
                const { User_ID, eventID, startTime, endTime } = req.query;
                console.log('User_ID:', User_ID, 'eventID:', eventID, 'startTime:', startTime, 'endTime:', endTime);
                ...
                const filteredTransactions = attendanceTransactions.filter(transaction => {
                    ...
                    if (startTime) {
                        ...
                        matches = matches && output.timestamp >= parsedStartTime;
                    }
                    if (endTime) {
                        ...
                        matches = matches && output.timestamp <= parsedEndTime;
                    }
