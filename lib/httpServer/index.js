...
class HttpServer {
    constructor(node, blockchain, operator, miner) {
        ...
        this.app.post('/operator/registerStudent', async (req, res) => {
            ...
            try {
                const result = await operator.Student_register(studentID, password);
        ...
        this.app.post('/operator/attendance', (req, res) => {
            const { studentId, eventId, timestamp, signature } = req.body;
        
            if (!studentId || !eventId || !timestamp || !signature) {
            ...
            try {
                const transactionId = blockchain.addTransaction(attendanceTransaction);  
